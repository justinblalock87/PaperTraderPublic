...
public struct PickerPlus<Data, Content> : View where Data: Hashable, Content: View {
    ...
    public var body: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: cornerRadius ?? 6.0).fill(
                backgroundColor
            )
        )
    }
...
struct CustomPicker: View {
    ...
    @Binding var selectedItem: StockPageSegments
    
    var body: some View {
        VStack {
            PickerPlus(
                StockPageSegments.allCases,
...
enum StockPageSegments: String, CaseIterable, Identifiable {
    case Summary
    case Chat
    case Learn
